var Timer = {
	...
	init:function (reserver,infoReservation,reserverCanvas,lastTimes,minutes) {
	...
	dataJcdecaux: function() {
		ajaxGet("https://api.jcdecaux.com/vls/v1/stations?contract=Lyon&apiKey=d9ce4a4ff8aee76cf49b9b8394047ea940b54c4d",function(reponse){
			...
			adresse = adresse.split(":"); // Découpage de l'adresse en deux partie.
			adresse = adresse[1];
			lengthMax = adresse.length; 
			adresse = adresse.substring(1,lengthMax); // Supression de l'espace.
	...
	diminuerCompteur: function() {
		...
		if((objTimer.secondes === 00) & (objTimer.minutes === 00)) {
			// Mise à jour du DOM
			objTimer.infoReservationElt.textContent = "";  
			...
			// Mise à jour du DOM
			objTimer.infoReservationElt.textContent = "Vélo réservé à la station " +  objTimer.nameStation + " par " + objTimer.nom + " " + objTimer.prenom;  
